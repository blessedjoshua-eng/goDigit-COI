...
def _get_session() -> requests.Session:
    if not hasattr(_thread_local, "session"):
        s = requests.Session()
        a = HTTPAdapter(pool_connections=10, pool_maxsize=10, max_retries=Retry(total=0))
        s.mount("https://", a)
        s.mount("http://",  a)
...
def get_identity() -> tuple:
    """Returns (user_id, company_code) from cache or SSM."""
    cache = read_cache()
    user_id      = cache.get("user_id", "")
    company_code = cache.get("company_code", "LI")
    if user_id:
        return user_id, company_code
    try:
        ssm          = _get_ssm_client()
        user_id      = ssm.get_parameter(Name=SSM_USER_ID)["Parameter"]["Value"]
        company_code = ssm.get_parameter(Name=SSM_COMPANY_CODE)["Parameter"]["Value"]
        return user_id, company_code
    except Exception:
        return "", "LI"
...
def validate_token(access_token: str) -> bool:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            log.info(f"  [Token Check] ✗ Token rejected by server [{resp.status_code}].")
            return False
    except Exception as e:
        log.info(f"  [Token Check] ✗ Validation error: {e}")
        return False
            ...
            try:
                ...
                if refresh_token and cookies:
                    ...
                    if resp.status_code == 200:
                        ...
                        log.warning(f"[TokenKeeper] Refresh failed [{resp.status_code}].")

                self._ready.set()

            except Exception as e:
                log.error(f"[TokenKeeper] Error: {e}")
                self._ready.set()
...
class Results:
    ...
    def print_summary(self):
        ...
        log.info("=" * 55)
        log.info(f"  Total: {total}  |  Success: {success}  |  Failed: {total - success}")
        log.info("=" * 55)
        for r in self.rows:
            if r["Status"] != "SUCCESS":
...
def _process_row(args):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            log.info(f"  [{loan}] ✓ Policy number: {policy_num}")

            # Download PDF
            log.info(f"  [{loan}] Downloading COI PDF for policy {policy_num}...")
            token     = get_token_fn()
            ...
            log.info(f"  [{loan}] ✓ PDF saved: {filename} ({size_kb:.1f} KB)")
    ...
    for batch_num, batch in enumerate(batches, 1):
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(futures):
                ...
                log.info(f"  Progress: {completed}/{total}  |  Success: {success_so_far}  |  Failed: {completed - success_so_far}")

        if batch_num < total_batches:
            log.info(f"  Batch {batch_num} done. Pausing {batch_pause}s...")
...
def main():
    ...
    # ── Step 1: Validate token — auto OTP login if needed ──────
    access_token = get_valid_token(mode)

    # ── Step 2: Token getter for downloader rows ───────────────
    def get_token_fn():
        """Re-checks token before each row — refreshes if near expiry."""
        cache      = read_cache()
        expires_at = cache.get("expires_at", 0)
        if time.time() < (expires_at - 60):
            return cache.get("access_token", access_token)
        return get_valid_token(mode)
