...
CONFIG = {
    ...
    # ── Login credentials ──────────────────────────────────────
    # Your GoDigit employee/web user ID (the username you type
    # on the login page — e.g. "96246062")
    "username": "96246062",

    # Your registered mobile number (10-digit, receives OTP SMS)
    "mobile": "8976624081",
...
def validate_token(access_token: str) -> bool:
    ...
    try:
        ...
        if resp.status_code == 200:
            log.info("  [Token Check] [OK] Token is valid.")
            return True
        elif resp.status_code == 401:
            log.info("  [Token Check] [FAIL] Token rejected [401 Unauthorized] - session expired or cookies dead.")
            return False
        elif resp.status_code == 403:
            log.info("  [Token Check] [FAIL] Token rejected [403 Forbidden] - insufficient permissions.")
            return False
        else:
            log.info(f"  [Token Check] [FAIL] Token rejected [{resp.status_code}] - unexpected response.")
            return False
    except requests.exceptions.Timeout:
        log.info("  [Token Check] [FAIL] Validation timed out - assuming token invalid.")
        return False
    except requests.exceptions.ConnectionError:
        log.info("  [Token Check] [FAIL] No internet connection - assuming token invalid.")
        return False
    except Exception as e:
        log.info(f"  [Token Check] [FAIL] Validation error: {type(e).__name__}: {e}")
        return False
...
def _get_username_form(session, code_challenge) -> tuple:
    ...
    # ── DEBUG: Save first page HTML to file so we can inspect it
    debug_path = Path(__file__).parent / "debug_login_page.html"
    debug_path.write_text(resp.text, encoding="utf-8")
    log.info(f"[DEBUG] First login page saved to: {debug_path}")

    soup  = BeautifulSoup(resp.text, "html.parser")

    # ── DEBUG: Log all forms found on this page
    all_forms = soup.find_all("form")
    log.info(f"[DEBUG] Forms found on login page: {[f.get('id', 'no-id') for f in all_forms]}")

    # ── DEBUG: Log all input fields found
    all_inputs = soup.find_all("input")
    log.info(f"[DEBUG] Input fields: {[(i.get('name'), i.get('type'), i.get('id')) for i in all_inputs]}")
    ...
    log.info(f"[DEBUG] Selected form id: {form.get('id', 'no-id')}")
    log.info(f"[DEBUG] Form action URL: {action}")
    log.info(f"[DEBUG] Hidden fields: {hidden}")
    log.info(f"[DEBUG] Final URL after redirects: {resp.url}")
...
def _submit_mobile_get_otp_form(session, action, hidden, referer) -> tuple:
    """
    Step 1: POST mobile number + first-form flag to trigger OTP SMS.
            The first page has an empty phone field + send-otp button.
            We fill in the phone and submit to trigger OTP.

    Step 2: Server sends OTP to phone and returns the same otp-form
            but now with phone pre-filled and a new session_code.
            This is the form we use to submit the OTP in the next step.
    """
    # ── Step 1: Submit mobile number to trigger OTP ─────────────
    payload = {
        **hidden,
        "phone":      CONFIG["mobile"],   # mobile number into phone field
        "username":   CONFIG["username"], # employee ID
    }

    log.info(f"[DEBUG] Step 1 - Triggering OTP. POSTing to: {action}")
    log.info(f"[DEBUG] Step 1 - Payload fields: {list(payload.keys())}")
    log.info(f"[DEBUG] Step 1 - phone={payload.get('phone')}  username={payload.get('username')}")
    ...
    log.info(f"[DEBUG] Step 1 response status: {resp.status_code}")
    log.info(f"[DEBUG] Step 1 response URL: {resp.url}")

    # Save debug page
    debug_path2 = Path(__file__).parent / "debug_otp_page.html"
    debug_path2.write_text(resp.text, encoding="utf-8")
    log.info(f"[DEBUG] Response page saved to: {debug_path2}")
    ...
    # ── DEBUG: Log all forms on response page ───────────────────
    all_forms2 = soup.find_all("form")
    log.info(f"[DEBUG] Forms on response page: {[f.get('id', 'no-id') for f in all_forms2]}")

    # ── Check for send-otp button disabled = OTP was sent ───────
    send_otp_btn = soup.find("input", id="otp-sent") or soup.find("input", id="send-otp")
    if send_otp_btn:
        log.info(f"[DEBUG] OTP button found: id={send_otp_btn.get('id')} value={send_otp_btn.get('value')} disabled={send_otp_btn.get('disabled')}")

    # ── Get OTP form for next step ───────────────────────────────
    otp_form = soup.find("form", id="otp-form")
    if not otp_form:
        error = soup.find(class_="pf-c-alert__title") or soup.find(id="input-error")
        raise RuntimeError(f"OTP form not found after mobile submit: {error.get_text(strip=True) if error else resp.text[:300]}")
    ...
    # Phone value — use CONFIG mobile since page may not pre-fill it
    phone_inp = soup.find("input", {"name": "phone"}) or soup.find("input", id="phone")
    phone     = phone_inp.get("value", "") if phone_inp else ""
    if not phone or phone == "xxxxxxxxxx":
        phone = CONFIG["mobile"]  # fallback to config value
        log.info(f"[DEBUG] Phone not in page — using CONFIG mobile: {phone}")
    else:
        log.info(f"[DEBUG] Phone from page: {phone}")

    log.info(f"[DEBUG] OTP form action: {otp_action}")
    log.info(f"[DEBUG] OTP hidden fields: {otp_hidden}")
...
def do_otp_login() -> dict:
    ...
    while True:
        ...
        try:
            ...
            log.info("[Login] Stage 1/5 - [OK] Login page loaded.")

            # ── Stage 2: Submit mobile -> trigger OTP ──────────
            print(f"  [2/5] Submitting username '{CONFIG['username']}'. OTP -> mobile ...{CONFIG['mobile'][-4:]}...")
            log.info(f"[Login] Stage 2/5 - Submitting username '{CONFIG['username']}' to trigger OTP...")
            otp_action, otp_hidden, phone, otp_referer = _submit_mobile_get_otp_form(
                session, action, hidden, referer
            )
            log.info(f"[Login] Stage 2/5 - [OK] OTP sent to mobile ending in ...{phone[-4:]}.")
            ...
            while True:
                ...
                if otp.isdigit() and len(otp) == 6:
                    break
                print("         [FAIL] Invalid. Please enter a 6-digit OTP.")
            ...
            log.info("[Login] Stage 4/5 - [OK] OTP verified. Auth code received.")
            ...
            log.info(f"[Login] Stage 5/5 - [OK] Tokens generated. Expires in {token_data.get('expires_in')}s.")
            ...
            log.info(f"[Login] [OK] {cookie_count}/6 session cookies captured.")

            print()
            print("  " + "=" * 48)
            print(f"  [OK] Login successful!")
            print(f"  [OK] Access token generated (valid 15 min)")
            print(f"  [OK] Refresh token generated (Lambda rotates every 10 min)")
            print(f"  [OK] {cookie_count} session cookies captured (valid ~3 days)")
            print(f"  [OK] User ID: {user_id}  |  Company: {company_code}")
            print("  " + "=" * 48)
            log.info("=" * 55)
...
def _save_credentials(data: dict, mode: str):
    ...
    write_cache(data)
    print(f"  [OK] Credentials saved to local cache.")
    log.info(f"[Save] [OK] Credentials saved to: {TOKEN_CACHE_FILE}")
    ...
    if mode == "prod":
        ...
        print("  [OK] Credentials pushed to AWS SSM. Lambda will pick up new refresh_token.")
        log.info("[Save] [OK] Credentials saved to SSM. Lambda will rotate token every 10 min.")
...
def get_valid_token(mode: str) -> str:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if new_token and validate_token(new_token):
                    ...
                    log.info("[Token] [OK] Token refreshed and validated successfully.")
                    # Update cache
                    if mode == "local":
                        cache = read_cache()
                    ...
                    print("  [FAIL] Token refresh failed - cookies have expired.")
                    log.warning("[Token] [FAIL] Refreshed token failed API validation. Cookies likely expired.")
            else:
                print(f"  [FAIL] Token refresh API returned [{resp.status_code}] - cookies likely expired.")
                log.warning(f"[Token] [FAIL] Token refresh API returned [{resp.status_code}]. Cookies likely expired.")
        except requests.exceptions.Timeout:
            print("  [FAIL] Token refresh timed out - GoDigit server not responding.")
            log.warning("[Token] [FAIL] Token refresh timed out (>30s). Server may be down.")
        except requests.exceptions.ConnectionError:
            print("  [FAIL] Token refresh failed - no internet connection.")
            log.warning("[Token] [FAIL] Token refresh failed - connection error. Check internet.")
        except Exception as e:
            print(f"  [FAIL] Token refresh error: {e}")
            log.warning(f"[Token] [FAIL] Token refresh unexpected error: {type(e).__name__}: {e}")
    ...
    errors = []
    if not CONFIG.get("username") or not CONFIG["username"].strip():
        errors.append("'username' not set in CONFIG - enter your GoDigit login ID (e.g. 96246062)")
    if not CONFIG.get("mobile") or len(CONFIG["mobile"].strip()) != 10 or not CONFIG["mobile"].strip().isdigit():
        errors.append("'mobile' not set correctly in CONFIG - enter your 10-digit registered mobile number")
    if errors:
        log.error("=" * 55)
        log.error("  ERROR: CONFIG incomplete. Fix these:")
        for e in errors:
            log.error(f"    - {e}")
        log.error("  Open godigit_coi.py and fill in the CONFIG section.")
        log.error("=" * 55)
        sys.exit(1)
            ...
            try:
                ...
                if refresh_token and cookies:
                    ...
                    if resp.status_code == 200:
                        ...
                        write_cache(cache)
                        print(f"  [Background] [OK] Token refreshed automatically. Expires in {data.get('expires_in')}s.")
                        log.info(f"[TokenKeeper] [OK] Token refreshed. Expires in {data.get('expires_in')}s.")
                    else:
                        body = resp.text[:150] if resp.text else "no body"
                        if resp.status_code == 400:
                            log.warning(f"[TokenKeeper] [FAIL] Refresh failed [400 Bad Request] - refresh token likely expired or revoked. Body: {body}")
                        elif resp.status_code == 401:
                            log.warning(f"[TokenKeeper] [FAIL] Refresh failed [401 Unauthorized] - cookies expired. OTP login needed. Body: {body}")
                        else:
                            log.warning(f"[TokenKeeper] [FAIL] Refresh failed [{resp.status_code}]. Body: {body}")

                self._ready.set()

            except Exception as e:
                if isinstance(e, requests.exceptions.Timeout):
                    log.error("[TokenKeeper] [FAIL] Timeout - GoDigit server did not respond within 30s.")
                elif isinstance(e, requests.exceptions.ConnectionError):
                    log.error("[TokenKeeper] [FAIL] Connection error - check internet connectivity.")
                else:
                    log.error(f"[TokenKeeper] [FAIL] Unexpected error: {type(e).__name__}: {e}")
                self._ready.set()
...
class Results:
    ...
    def print_summary(self):
        ...
        log.info("=" * 55)
        log.info(f"  Total: {total}  |  [OK] Success: {success}  |  [SKIP] Skipped: {skipped}  |  [FAIL] Failed: {failed}")
        log.info("=" * 55)
        if failed:
...
def _process_row(args):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            log.info(f"  [{loan}] [OK] Policy: {policy_num}")
            ...
            if os.path.exists(filepath) and os.path.getsize(filepath) > 1024:
                ...
                log.info(f"  [{loan}] [SKIP] Already downloaded: {filename} ({size_kb:.1f} KB) - skipping.")
            ...
            log.info(f"  [{loan}] [OK] Saved: {filename} ({size_kb:.1f} KB)")
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(futures):
                ...
                log.info(
                    ...
                    f"  | [OK] {success_so_far}"
                    f"  | [SKIP] {skipped_so_far} skipped"
                    f"  | [FAIL] {failed_so_far} failed"
                )
